...
inject_shared_css()
render_header("CoreFlood Lab — Permeability Matching Tool")
...
def _apply_layout(fig, t_max, y_max, title):
    fig.update_layout(
        title=dict(text=title, font=dict(family="Courier New",
                                         color="#111827", size=14)),
        xaxis=dict(
...
def build_empty_chart(t_max, target_dp):
    ...
    _apply_layout(fig, t_max, max(target_dp * 1.25, 1.0),
                  "Awaiting simulation…")
    return fig
...
with tab_abs:
    ...
    with left:
        st.markdown('<div class="section-label">▌ INPUT PARAMETERS</div>',
                    unsafe_allow_html=True)

        st.markdown('<div class="group-label">Core Geometry</div>',
                    unsafe_allow_html=True)
        ...
        p_val,  p_unit  = input_row("Back pressure", 10.0,
                                    PRESSURE_TO_BAR, "p")
        dp_val, dp_unit = input_row("Target ΔP",     50.0,
                                    DP_TO_MBAR,      "dp")
        t_val,  t_unit  = input_row("Sim time",      60.0,
                                    TIME_TO_MIN,     "t",
                                    default_unit="min")
    ...
    with right:
        st.markdown('<div class="section-label">▌ MATCHING RESULT</div>',
                    unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-label">▌ DIFFERENTIAL PRESSURE vs TIME</div>',
                unsafe_allow_html=True)
